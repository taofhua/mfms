      ...
      <el-container>
        <el-header style="height: 200px;">
            ...
            <div>
              ...
              <el-link href="#" style="margin-left: 10px;">购物车[0]</el-link>
            </div>
          </div>
  ...
  <el-menu-item v-for="subCategory in typeData.children" :key="subCategory.id" :index="subCategory.id.toString()" @click="navTo('/type?id='+subCategory.id)">
   ...
   <el-breadcrumb-item>{{goodsData.name}}</el-breadcrumb-item>
      ...
      <el-col :span="6" v-for="item in searchResults" :key="item.id">
        <img @click="navTo('/detail?id=' + item.id)" :src="getImageUrl(item.iconUrl)" alt="" style="width: 100%; height: 175px; border-radius: 10px; border: #cccccc 1px">
...
export default {
  setup() {
    ...
    const fetchSearchResults = async (query) => {
      try {
        const response = await axios.get('http://192.168.153.25:9093/api/v1/search', { params: { q: query } });
    ...
    async loadGoods() {
      try {
        const response = await axios.get(
          "http://192.168.153.25:9093/api/v1/product/hot?num=1"
        );
